...
namespace boost { namespace spirit { namespace qi
{
    ...
    struct action : unary_parser<action<Subject, Action> >
    {
        ...
        {
            typedef typename attribute<Context, Iterator>::type my_attr_type;
            typedef traits::make_attribute<my_attr_type, Attribute> make_attribute;

            // create an attribute if one is not supplied
            typedef traits::transform_attribute<
                typename make_attribute::type, my_attr_type, domain> transform;

            typename make_attribute::type made_attr = make_attribute::call(attr_);
            typename transform::type my_attr = transform::pre(made_attr);

            Iterator save = first;
            if (subject.parse(first, last, context, skipper, my_attr))
            {
                // call the function, passing the attribute, the context.
                // The client can return false to fail parsing.
                if (traits::action_dispatch<Subject>()(f, my_attr, context)) 
                {
                    // Do up-stream transformation, this integrates the results
                    // back into the original attribute value, if appropriate.
                    traits::post_transform(attr_, my_attr);
                    return true;
                }
