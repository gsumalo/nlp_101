...
#undef BOOST_SPIRIT_LOG2
    ...
    struct positive_accumulator
    {
        ...
        {
            const int my_digit = radix_traits<Radix>::digit(ch);
            n = n * T(Radix) + T(my_digit);
        }
        ...
        inline static bool add(T& n, Char ch, mpl::true_) // checked add
        {
            ...
            const int my_digit = radix_traits<Radix>::digit(ch);
            if (n > max::value - my_digit)
                return false;

            n += static_cast<T>(my_digit);
            return true;
        }
    };
    ...
    struct negative_accumulator
    {
        ...
        inline static bool add(T& n, Char ch, mpl::true_) // checked subtract
        {
            ...
            int const my_digit = radix_traits<Radix>::digit(ch);
            if (n < min::value + my_digit)
                return false;
